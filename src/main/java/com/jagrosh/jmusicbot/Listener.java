...
public class Listener extends ListenerAdapter {
    ...
    public void onReady(ReadyEvent event) {
        if (event.getJDA().getGuilds().isEmpty()) {
            ...
            log.warn("This bot is not in a group! Use the link below to add the bot to your group.");
    ...
    public void onGuildVoiceJoin(@NotNull GuildVoiceUpdateEvent event) {
        ...
        log.debug("Regeneration and restart judgment {}", ((event.getChannelJoined().getMembers().size() > 1 && event.getChannelJoined().getMembers().contains(botMember)) && Objects.requireNonNull(handler).getPlayer().isPaused()));
        //Is there at least one person in the voice chat, is a bot in the voice chat, and is playback paused?
