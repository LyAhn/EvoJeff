...
public class BotConfig {
    ...
    public void load() {
        ...
        try {
			...
			// validate bot token
             if (token == null || token.isEmpty() || token.matches("(BOT_TOKEN_HERE|Paste your Bot token here)")) {
                 token = prompt.prompt("Please enter your BOT token."
                         + "\nHere's how to get a token:"
                         + "\nhttps://github.com/jagrosh/MusicBot/wiki/Getting-a-Bot-Token."
                         + "\nBOT token: ");
                 if (token == null) {
                     prompt.alert(Prompt.Level.WARNING, CONTEXT, "No token entered! Exiting.\n\nConfiguration file location: " + path.toAbsolutePath());
                     return;
                 } else {
                     write = true;
                 }
            ...
            if (owner <= 0) {
                try {
                    owner = Long.parseLong(prompt.prompt("Owner user ID is not set or is not a valid ID."
                            + "\nPlease enter the user ID of the BOT owner."
                            + "\nClick here to get your user ID:"
            ...
            if (write) {
                ...
                if (original == null) {
                    ...
                    mod = original.substring(original.indexOf(START_TOKEN) + START_TOKEN.length(), original.indexOf(END_TOKEN))
                            .replace("BOT_TOKEN_HERE", token).replace("Paste your Bot token here", token)
                            .replace("0 // OWNER ID", Long.toString(owner)).replace("Paste owner ID here", Long.toString(owner))
                            .trim();
                }
