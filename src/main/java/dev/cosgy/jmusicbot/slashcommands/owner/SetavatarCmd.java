...
public class SetavatarCmd extends OwnerCommand {
     public SetavatarCmd(Bot bot) {
         this.name = "setavatar";
         this.help = "Set your bot's avatar";
         this.arguments = "<url>";
         this.aliases = bot.getConfig().getAliases(this.name);
         this.guildOnly = false;
         List<OptionData> options = new ArrayList<>();
         options.add(new OptionData(OptionType.STRING, "image", "image URL", true));
     ...
     protected void execute(SlashCommandEvent event) {
         ...
         if (s == null) {
             event.reply(event.getClient().getError() + "Invalid or missing URL").queue();
         } else {
             try {
                 event.getJDA().getSelfUser().getManager().setAvatar(Icon.from(s)).queue(
                         v -> event.reply(event.getClient().getSuccess() + "Your avatar has been changed.").queue(),
                         t -> event.reply(event.getClient().getError() + "Failed to set avatar.").queue());
             } catch (IOException e) {
                 event.reply(event.getClient().getError() + "Unable to read from the URL provided.").queue();
             }
         }
     }
         ...
         if (s == null) {
             ...
             try {
                 event.getSelfUser().getManager().setAvatar(Icon.from(s)).queue(
                         v -> event.reply(event.getClient().getSuccess() + "Your avatar has been changed."),
                         t -> event.reply(event.getClient().getError() + "Failed to set avatar."));
             } catch (IOException e) {
                 event.reply(event.getClient().getError() + "Unable to read from the URL provided.");
             }
         }
     }
}