...
public class LyricsCmd extends MusicCommand {
    ...
    public void doCommand(SlashCommandEvent event) {
        ...
        {
            if (lyrics == null) {
                event.reply(event.getClient().getError() + "`" + title + "No lyrics for ` were found." + (event.getOption("name").getAsString().isEmpty() ? " Try entering the song name manually (`lyrics [song name]`)" : "")).queue();
                return;
            }
            ...
            if (lyrics.getContent().length() > 15000) {
                event.reply(event.getClient().getWarning() + " `" + title + "` Song found with lyrics, but may be incorrect: " + lyrics.getURL()).queue( );
    ...
    public void doCommand(CommandEvent event) {
        ...
        if (event.getArgs().isEmpty()) {
            ...
            if (sendingHandler.isMusicPlaying(event.getJDA()))
                ...
                event.replyError("The song cannot be used because it is not playing.");
                return;
            }
        } else
            title = event.getArgs();
        ...
        {
            if (lyrics == null) {
                event.replyError("`" + title + "No lyrics were found for `." + (event.getArgs().isEmpty() ? " Try entering the song name manually (`lyrics [song title]` )" : ""));
                return;
            }
            ...
            EmbedBuilder eb = new EmbedBuilder()
                    ...
                    .setTitle(lyrics.getTitle(), lyrics.getURL());
            if (lyrics.getContent().length() > 15000) {
                event.replyWarning(" A song was found with the lyrics `" + title + "`, but they may be incorrect: " + lyrics.getURL());
            } else if (lyrics.getContent().length() > 2000) {
                String content = lyrics.getContent().trim();
                while (content.length() > 2000) {
                    int index = content.lastIndexOf("\n\n", 2000);
