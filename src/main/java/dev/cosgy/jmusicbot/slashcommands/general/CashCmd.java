...
public class CashCmd extends SlashCommand {
    ...
    public CashCmd(Bot bot) {
        this.bot = bot;
        this.name = "cache";
        this.help = "Display songs stored in cache.";
        this.guildOnly = true;
    ...
    protected void execute(CommandEvent event) {
        if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
            event.reply("There were no songs saved in the cache.");
            return;
        }
        int pagenum = 1;
        try {
    ...
    private String getQueueTitle(String success, int songsLength, long total) {
        ...
        return FormatUtil.filter(sb.append(success).append(" List of songs saved in cache | ").append(songsLength)
                .append(" song | `").append(FormatUtil.formatTime(total)).append("` ")
                .toString());
    }
    ...
    public static class DeleteCmd extends DJCommand {
        public DeleteCmd(Bot bot) {
            ...
            this.help = "Delete the saved cache.";
            this.guildOnly = true;
        }
        ...
        public void doCommand(CommandEvent event) {
            if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                event.reply("Cache does not exist.");
                return;
            }
            ...
            event.reply("I deleted the cache.");
        }

        @Override
        public void doCommand(SlashCommandEvent event) {
            if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                event.reply("Cache does not exist.").queue();
                return;
            }
            ...
            event.reply("I deleted the cache.").queue();
        }
    }
    ...
    public class ShowCmd extends SlashCommand {
        ...
        protected void execute(SlashCommandEvent event) {
            if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                event.reply("There were no songs saved in the cache.").queue();
                return;
            }
            int pagenum = 1;
            event.reply("Get cache.").queue();
